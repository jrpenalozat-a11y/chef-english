...
5. **Pantalla de detalle por día (Curso)** — ✅ HECHO en parte: al tocar un día se abre su propia pantalla (5 frases) con botón «← Volver a los días» y regreso automático a la grilla al completar el día. Bajo cada frase, desplegable **«💡 Otras formas de decirlo»** con **usos alternativos** (2 por frase: frase + pronunciación + traducción + matiz formal/casual). Contenido cargado para **días 1–5 en en/pt/zh** (objeto `ALT`). **Pendiente:** extender los usos alternativos a los **días 6–30** en los 3 idiomas (misma estructura `ALT`).
6. **Gamificación de las frases alternativas (idea futura):** al aprender una frase alternativa, mostrar un **marcador/insignia de racha** tipo «🚀 vas como bala» / «🔥 on fire» (o similar) con íconos representativos; al ir avanzando, una **barra/medidor paralelo de fuego** (cuenta solo las alternativas aprendidas). Y al **terminar de aprender TODAS las frases nuevas** (las 2 extra de cada frase del día/curso), una **súper-hiper felicitación** con una frase motivadora divertida (reusar el overlay `celebrate()` + arrays `CEL`/`pickFun` con un set especial). Viable y sin backend: contador de racha en `localStorage` + insignias + barra extra de progreso.
